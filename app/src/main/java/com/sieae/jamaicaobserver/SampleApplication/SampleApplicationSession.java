...
package com.sieae.jamaicaobserver.SampleApplication;
...
public class SampleApplicationSession implements UpdateCallbackInterface
{
    ...
    // Scale values to compensate video background for differences in aspect ratio between camera and screen
    private Vec2F mVbScale;
    ...
    public void startAR(int camera) throws SampleApplicationException
    {
        ...
        int forcedVRFrameRate = -1;
        if (!Renderer.getInstance().setTargetFps(forcedVRFrameRate))
        {
            Log.e(LOGTAG, "Failed to set rendering frame rate to: " + forcedVRFrameRate + " fps");
            return;
        }
        else
        {
            Log.e(LOGTAG, "Configured frame rate set to recommended frame rate: " + forcedVRFrameRate + " fps");
        }
    ...
    public Vec2F getVideoBackgroundScale()
    {
        return mVbScale;
    }
    ...
    private class InitVuforiaTask extends AsyncTask<Void, Integer, Boolean>
    {
        ...
        protected Boolean doInBackground(Void... params)
        {
            ...
            synchronized (mShutdownLock)
            {
                Vuforia.setInitParameters(mActivity, mVuforiaFlags, GlobalVariables.VUFORIA_LICENSE_KEY);
    ...
    public void setProjectionMatrix()
    {
        ...
        mProjectionMatrix = Tool.getProjectionGL(camCal, NEAR_PLANE, FAR_PLANE);
    }
    ...
    private void configureVideoBackground()
    {
        ...
        // Retain the scale factors so we don't need to repeat this calculation elsewhere
        float vbScaleX = (float)xSize / mScreenWidth;
        float vbScaleY = (float)ySize / mScreenHeight;
        mVbScale = new Vec2F(vbScaleX, vbScaleY);
