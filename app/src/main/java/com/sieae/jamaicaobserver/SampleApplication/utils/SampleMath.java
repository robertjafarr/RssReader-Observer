...
public class SampleMath
{
    ...
    public static Matrix44F multiplyMatrix(Matrix44F matrixA, Matrix44F matrixB)
    {
        int i, j, k;
        Matrix44F r = new Matrix44F();
        ...
        for (i = 0; i < 4; i++)
        {
            for (j = 0; j < 4; j++)
            {
                ...
                for (k = 0; k < 4; k++)
                    temp[j * 4 + i] += matrixA.getData()[k * 4 + i] * matrixB.getData()[j * 4 + k];
            }
        }
        r.setData(temp);
        return r;
    ...
    public static Matrix44F Matrix44FRotate(float angle, Vec3F axis, Matrix44F m)
    {
        Matrix44F r, rotationMatrix;

        r = new Matrix44F(m);
        // create a rotation matrix
        rotationMatrix = makeRotationMatrix(angle, axis);
        r = multiplyMatrix(r, rotationMatrix);

        return r;
    }


    public static Matrix44F makeTranslationMatrix(Vec3F trans)
    {
        Matrix44F r = Matrix44FIdentity();

        for (int i = 0; i < 16; i++)
            temp[i] = 0.0f;

        temp[12] = trans.getData()[0];
        temp[13] = trans.getData()[1];
        temp[14] = trans.getData()[2];
    ...
    public static Matrix44F makeRotationMatrix(float angle, Vec3F axis)
    {
        double radians, c, s, c1, length;
        ...
        length = Math.sqrt(axis.getData()[0] * axis.getData()[0] + axis.getData()[1] * axis.getData()[1] + axis.getData()[2] * axis.getData()[2]);

        u[0] = axis.getData()[0] / length;
        u[1] = axis.getData()[1] / length;
        u[2] = axis.getData()[2] / length;
        ...
        for (i = 0; i < 3; i++)
        {
            temp[i * 4 + (i + 1) % 3] = (float)(u[(i + 2) % 3] * s);
            temp[i * 4 + (i + 2) % 3] = (float)(-u[(i + 1) % 3] * s);
        }

        for (i = 0; i < 3; i++)
        {
            for (j = 0; j < 3; j++)
                temp[i * 4 + j] += (float)(c1 * u[i] * u[j] + (i == j ? c : 0.0));
        }
